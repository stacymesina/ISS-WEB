...
function highlightAdminNav() {
  if (SiteConfig.isAdmin()) {
    document.querySelectorAll(".staff-access").forEach((el) => {
      ...
      if (link) {
        link.textContent = "Editor (signed in)";
      }
    });
  }
}
...
function renderAboutParagraphs(cfg) {
  ...
  container.innerHTML = cfg.about.paragraphs
    .map(
      (p, index) => `
        <p style="margin-top:${index === 0 ? "0" : "1rem"}">
          ${escapeHtml(p)}
        </p>
      `
    )
    ...
    .map(
      (m) => `
        <div class="member">
          ...
          <img
            src="${escapeHtml(m.image || "images/default-profile.png")}"
            alt="${escapeHtml(m.role)}"
          >
...
function updateMembershipNav(cfg) {
  document
    .querySelectorAll('a[href="membership.html"]')
    .forEach((link) => {
